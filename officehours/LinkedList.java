...
public class LinkedList<T extends Comparable<T>>
{
	...
	public void insertInOrder(T  data)
	{
			...
			Node<T> curr = head;
			Node<T> prev = null;
			while(curr != null && cmpData.compareTo(curr.getData())>0)
			{
					//increment curr
					prev = curr;
					curr = curr.getNext();
	...
	{	//Walk list until you find 1st Node with this key.
		//current node and next node
		//if current node contains key return true
	...
	//	for ( curr = head; curr != null; curr = curr.getNext())
		while(curr!=null)
		{
			...
			// curr is equal to key we want here
				if(found == false)
				return false;
	...
	{	//this = set1 other = set2
		...
		//Node<T> currInOther = other.head;

		for (Node<T> curr = this.head; curr != null; curr = curr.getNext())
		{//increment currInUnion -- add the this set to union
			//other.head --
					// add to union the this list
					union.insertInOrder(curr.getData());
		}
		for(Node<T> currInOther = other.head; currInOther!= null; currInOther = currInOther.getNext())
		{		//if other does not contain this key
				//add to  union from other
			if(!this.contains( currInOther.getData() ))
				union.insertInOrder(currInOther.getData());
		}
		return union;
	}
